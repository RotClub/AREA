...
<div class="w-full h-full flex flex-col justify-center items-center">
	<form class="card w-full max-w-md flex flex-col items-center" method="post">
		<span class="w-full flex flex-row items-center justify-center text-3xl font-semibold mt-4 mb-2"
		...
		<div class="w-full h-full flex flex-col items-center">
			<div class="w-4/5 h-full py-2 flex flex-col gap-2">
				{#if !authToggle} <!-- register -->
					<div>
						<label for="username">Username</label>
						<input type="text" id="username" name="username" class="input" placeholder="gougougaga" />
					</div>
					<div>
		...
		<div class="w-4/5 flex flex-row justify-between items-center mb-4 mt-2">
			<div class="flex flex-row items-center gap-2">
				...
				<SlideToggle name="login" bind:checked={authToggle} active="bg-primary-500" background="bg-primary-500" />
